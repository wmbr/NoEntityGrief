...
public class NoEntityGrief extends JavaPlugin implements Listener { // Licensed
																	// under
																	// GPLv3
	...
	public void onBlockChange(EntityChangeBlockEvent e) {
		if (e.getEntity() instanceof Enderman
			...
			e.setCancelled(true);
				...
				&& EWs[5] != null && EWs[5].contains(e.getBlock().getLocation().getWorld().getName()))
			e.setCancelled(true);
		else if (e.getEntity() instanceof Sheep
				&& EWs[1] != null && EWs[1].contains(e.getBlock().getLocation().getWorld().getName())) {
			e.setCancelled(true);
	...
	public void onExplosion(EntityExplodeEvent e) {
		if (e.getEntity() instanceof TNTPrimed) {
			if (EWs[3] != null && EWs[3].contains(e.getLocation().getWorld().getName()))
				e.blockList().clear();
		} else if (EWs[2] != null && EWs[2].contains(e.getLocation().getWorld().getName()))
			e.blockList().clear();
	}
	...
	public void onPaintingBreak(HangingBreakEvent e) {
		if (e.getCause() == HangingBreakEvent.RemoveCause.EXPLOSION
			...
			if (EWs[3] != null && EWs[3].contains(e.getEntity().getWorld().getName()))
				e.setCancelled(true);
		} else if (e.getCause() == HangingBreakEvent.RemoveCause.EXPLOSION
				&& EWs[2] != null && EWs[2].contains(e.getEntity().getWorld().getName()))
			e.setCancelled(true);
	}
